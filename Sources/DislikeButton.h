...
/**
 ...
 `[AdaptrAudioPlayer unlike]`. When a user taps on the button while it is enabled and not
 selected, the button will call `[AdaptrAudioPlayer dislike]`.
 
 The button will listen for state change events from the `AdaptrAudioPlayer` and update its
 state accordingly.
...
//NOT_IB_DESIGNABLE
@interface DislikeButton : UIButton
